...
// Recap serale: inviato una volta al giorno alla prima raccolta dopo le 21:00
// (ora italiana), così arriva entro le 22. La finestra ha un limite superiore
// per non spedire un "recap della giornata" a notte fonda se i collect sono radi.
const RECAP_START_HOUR = 21;
const RECAP_END_HOUR = 23;

// Ora e data correnti nel fuso italiano (gestisce in automatico l'ora legale).
...
export async function maybeNotifyTopStory(topStory) {
  ...
  if (recent.some((r) => isSameNews(sig, r))) {
    await writeJson(STATE_FILE, { ...state, initialized: true, recent });
...
// Recap serale (daily digest): una sola notifica al giorno con i titoli
// principali della giornata. Inviata alla prima raccolta nella finestra serale
// (21:00–23:00 ora italiana); dedupe per data così non si ripete.
export async function maybeSendEveningRecap({ digest, articles } = {}) {
  const list = Array.isArray(articles) ? articles : [];
  if (!list.length) return { skipped: "no-articles" };

  const { date, hour } = romeNow();
  if (hour < RECAP_START_HOUR || hour >= RECAP_END_HOUR) {
    return { skipped: "not-time" };
  }

  const state = await readJson(STATE_FILE, {});
  if (state.lastRecapDate === date) return { skipped: "already-sent" };

  // Titoli principali della giornata: più momentum/copertura, poi più recenti.
  const top = [...list].sort(
    (a, b) =>
      (b.momentum || 1) - (a.momentum || 1) ||
      (b.coverage || 1) - (a.coverage || 1) ||
      new Date(b.lastAt || b.publishedAt || 0) -
        new Date(a.lastAt || a.publishedAt || 0)
  );
  const heads = top.slice(0, 3).map((a) => `• ${a.title}`);
  const extra = list.length - Math.min(3, list.length);
  const body =
    heads.join("\n") + (extra > 0 ? `\n…e altre ${extra} notizie` : "");

  const result = await sendToAll({
    title: "Lazio24 — la giornata biancoceleste",
    body,
    url: "/",
    tag: "lazio24-recap",
  });
  await writeJson(STATE_FILE, { ...state, lastRecapDate: date });
  console.log(`  recap serale inviato: ${result.sent} (rimossi: ${result.removed})`);
  return { sent: result.sent, removed: result.removed };
}
