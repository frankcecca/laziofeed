...
export async function groupStories(items = []) {
  ...
  const list = items
    ...
    .join("\n");
  const prompt = `Sei un redattore. Qui sotto c'è un elenco numerato di notizie sulla S.S. Lazio da varie testate. Raggruppa gli articoli che riguardano LO STESSO ARGOMENTO. Regole:
- Stesso argomento = stesso soggetto principale + stessa vicenda (es. una trattativa di mercato, un evento, un tema). Metti nello STESSO gruppo TUTTI gli articoli su quella vicenda, anche se sono aggiornamenti, dichiarazioni o opinioni diverse. Esempio: "offerta del Napoli per Gila", "accordo Gila-Napoli", "Tizio: Gila è fortissimo", "contropartita per Gila" → UN UNICO gruppo sulla trattativa Gila-Napoli.
- Tieni separati SOLO argomenti realmente diversi (giocatori, allenatori, vicende diverse). Nel dubbio se due articoli parlano della stessa vicenda, raggruppali.
- Includi nei gruppi solo articoli che hanno almeno un altro articolo sullo stesso argomento. Gli articoli su un argomento unico NON vanno in alcun gruppo.
- Per ogni gruppo scrivi un "titolo" (max ~12 parole, parole tue, neutro, senza virgolette) e una "descrizione" (UNA frase breve) che riflettano l'argomento del gruppo.
...
${list}`;
  try {
    const text = (await chat(prompt, 4000)) || "";
    const match = text.match(/\{[\s\S]*\}/);
    if (!match) return null;
    const obj = JSON.parse(match[0]);
