...
// Regola anti-invenzione, condivisa dai prompt che generano testo. Il modello
// tende ad "arrotondare" aggiungendo dettagli plausibili ma non presenti nella
// fonte: il caso tipico è il RUOLO del calciatore (es. dare del "difensore" a
// un esterno offensivo). Un dettaglio inventato mina la fiducia nel sito.
const NO_INVENT_RULE =
  'FEDELTÀ ALLA FONTE (regola CRITICA): NON aggiungere fatti, numeri, squadre, ' +
  'cifre o dettagli che non siano scritti nelle notizie qui sotto. In ' +
  'particolare NON attribuire a una persona un ruolo o una posizione ' +
  '(es. "difensore", "attaccante", "centrocampista", "esterno", "portiere", ' +
  '"allenatore") se quel ruolo NON è esplicitamente indicato nella fonte: in ' +
  'caso di dubbio usa SOLO il nome, senza ruolo. Non dedurre il ruolo dal nome.';
...
async function chat(prompt, maxTokens, modelOverride) {
  ...
  if (groqKey) {
    const model =
      modelOverride || process.env.SUMMARY_MODEL || "openai/gpt-oss-120b";
...
export async function synthesizeStory(title, members = []) {
  ...
  const lines = members
    ...
    .join("\n");
  const prompt = `Queste sono notizie da più testate sullo stesso fatto riguardante la S.S. Lazio. Genera UN SOLO titolo e UNA SOLA descrizione che facciano la sintesi delle diverse versioni, in italiano, in modo neutro e senza favorire una testata. Usa parole tue: NON copiare frasi o espressioni dalle fonti. Il titolo: max ~12 parole, incisivo, senza virgolette. La descrizione: UNA frase breve. ${NO_INVENT_RULE} PRUDENZA sul calciomercato (importante): tratta gli affari SEMPRE come ipotesi, al condizionale e con termini di trattativa ("idea", "contatti", "sondaggio", "obiettivo", "verso", "sarebbe vicino"); EVITA verbi da affare concluso ("fatta", "chiuso", "preso", "è del", "ufficiale") a meno che la fonte non lo dichiari esplicitamente ufficiale (es. "Gila verso il Napoli", mai "Gila al Napoli"). Rispondi SOLO con JSON valido: {"titolo":"...","descrizione":"..."}.\n\n${lines}`;
  try {
...
- ${NO_INVENT_RULE}
- PRUDENZA sul calciomercato (regola IMPORTANTE): tratta gli affari di mercato SEMPRE come ipotesi, mai come fatti compiuti. Default al condizionale e a termini di trattativa: "idea", "sondaggio", "contatti", "obiettivo", "piace", "verso", "in trattativa", "sarebbe vicino". EVITA i verbi che danno per concluso l'affare ("fatta", "chiuso", "preso", "è del", "acquistato", "ceduto", "ufficiale") A MENO CHE la fonte non lo dichiari ESPLICITAMENTE ufficiale/firmato. Esempi: "Gila, contatti col Napoli" o "Gila verso il Napoli", MAI "Gila al Napoli" / "fatta per Gila".
...
export async function summarize({ title, snippet }) {
  ...
  const prompt = `Scrivi in italiano UNA sola frase breve (max ~25 parole) che riassuma questa notizia sulla S.S. Lazio. Usa parole tue: NON copiare frasi o espressioni dal testo della fonte. ${NO_INVENT_RULE} Tono neutro, niente preamboli, niente virgolette.\n\nTitolo: ${title}\nTesto: ${clean}`;
  try {
