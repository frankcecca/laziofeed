...
export async function groupStories(items = []) {
  ...
  const list = items
    ...
    .join("\n");
  const prompt = `Sei un redattore. Qui sotto c'è un elenco numerato di notizie sulla S.S. Lazio da varie testate. Individua i GRUPPI di articoli che trattano LA STESSA IDENTICA notizia (stesso fatto/soggetto/evento). Regole:
- Raggruppa SOLO articoli realmente sullo stesso fatto. Soggetti diversi (giocatori, allenatori, eventi, trattative differenti) NON vanno insieme. Meglio lasciare separato che sbagliare.
- Includi nei gruppi SOLO articoli con almeno un altro che parla dello stesso fatto. Gli articoli unici NON vanno messi in nessun gruppo.
- Per ogni gruppo scrivi un "titolo" (max ~12 parole, parole tue, neutro, senza virgolette) e una "descrizione" (UNA frase breve) che riflettano SOLO i membri di quel gruppo.
