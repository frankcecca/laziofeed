...
// Token dei titoli ORIGINALI degli articoli-membro (non del titolo sintetizzato,
// che per costruzione è identico tra i candidati alla fusione). È qui che vivono
// i nomi propri che distinguono davvero una storia.
function memberTokens(s) {
  const set = new Set();
  for (const m of s._members || [])
    for (const t of tokenize(m.title || "")) set.add(t);
  return set;
}

// Unione di sicurezza DOPO la sintesi: due cluster distinti a volte ricevono un
// titolo sintetizzato (quasi) identico. Li fondiamo SOLO se, oltre alla chiave
// del titolo, condividono nei titoli originali un token RARO e lungo (un nome
// proprio: Provedel, Romagnoli…). Così si combinano le card davvero duplicate
// senza mischiare storie diverse — e i relativi link di fonte — capitate per
// caso sullo stesso titolo generico ("affare in dirittura", "le ultime"…).
function mergeDuplicateStories(stories) {
  // Document frequency dei token sui titoli originali, per stimare la rarità.
  const df = new Map();
  for (const s of stories)
    for (const t of memberTokens(s)) df.set(t, (df.get(t) || 0) + 1);
  const rareMax = Math.max(2, Math.ceil(stories.length * 0.15));
  const sameSubject = (a, b) => {
    for (const t of a)
      if (b.has(t) && t.length >= 5 && (df.get(t) || 0) <= rareMax) return true;
    return false;
  };

  const keep = [];
  const byKey = new Map();
  const pools = new Map(); // indice in keep -> Set di token (titoli originali)
  for (const s of stories) {
    const key = tokenize(s.title).sort().join(" ");
    const idx = key ? byKey.get(key) : undefined;
    const pool = memberTokens(s);

    if (key && idx !== undefined && sameSubject(pool, pools.get(idx))) {
      const primary = keep[idx];
      ...
      for (const t of pool) pools.get(idx).add(t); // arricchisci il pool
      continue;
    }

    // Non fondibile (chiave nuova, oppure stessa chiave ma soggetti diversi):
    // resta una storia a sé. La prima occorrenza della chiave fa da àncora.
    if (key && idx === undefined) byKey.set(key, keep.length);
    pools.set(keep.length, pool);
    keep.push(s);
  }
  return keep;
}
