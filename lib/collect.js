...
// titolo sintetizzato (quasi) identico. Per non mostrare card duplicate li
// fondiamo per parole chiave del titolo: combina fonti e copertura, alza il
// momentum al massimo dei due e tiene il più recente. Non altera la soglia del
// clustering (resta prudente sui titoli originali).
function mergeDuplicateStories(stories) {
  ...
  for (const s of stories) {
    ...
    if (!key || idx === undefined) {
      if (key) byKey.set(key, keep.length);
      keep.push(s);
      continue;
    }
