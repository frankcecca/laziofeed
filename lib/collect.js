...
export async function collect() {
  ...
  // Raggruppamento semantico (paradigma B): è l'LLM a capire quali articoli
  // trattano la stessa notizia. Se l'AI non risponde o l'output non è valido,
  // si ripiega sull'euristica per parole del titolo (clusterStories).
  ...
  if (aiGroups) {
    ...
    for (const g of aiGroups) {
      const fresh = g.members
        .map((i) => pool[i])
        .filter((m) => m && !assigned.has(m.id));
      if (fresh.length < 2) continue;
      fresh.forEach((m) => assigned.add(m.id));
      const story = buildStory(fresh);
      story._aiTitle = g.title; // titolo/sintesi proposti dal raggruppamento AI
      story._aiSummary = g.summary;
      built.push(story);
    }
    // Articoli non raggruppati → storie singole.
    for (const a of pool) {
    ...
    stories = built;
    console.log(`  raggruppamento AI: ${built.length} storie`);
  } else {
  ...
  await mapPool(orderedSingles, 2, async (s) => {
